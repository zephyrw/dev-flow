...
状态：`done`；预计 1.5 人日；未通过不得删除旧实现或切换生产路径。

**实现提交：** W01 全部验收通过。
- AC01: koffi@3.3.1 安装并加载成功 ✓
- AC02: 无预分配副作用 ✓
- AC09: Mutex/Job 互斥原语可用 ✓
- runner-entry IPC 协议测试通过 (ready→start→started→exited)
- process-protocol 单元测试 17/17 通过
- 原生接口集成测试 8/1 通过 (1 POSIX-only 跳过)
...
状态：`done`；预计 2 人日；依赖 W01。

**实现提交：** W02 核心改造完成。
- manager.ts: 移除 hostExecutable/requireHost，统一为直接 spawn
- controller-lock.ts: 改为进程内 Mutex/flock，不产生子进程
- runtime.ts: 构造函数不再传 hostExecutable
- main.ts, accounts-main.ts: 使用新锁签名
- process-protocol.ts 在 W01 已创建
...
状态：`done`；预计 1.5 人日；依赖 W02。

**实现提交：** W03 核心改造完成。
- agy-account-job-runner.ts: 移除 hostExecutable，所有方法直接 spawn
- agy-account-processes.ts: 移除 hostExecutable，用 native 模块检查进程状态
- recovery.ts: 用 native 模块替代 host executable 检查进程存活
- account-service-bootstrap.ts: 移除 hostExecutable 参数
- main.ts/accounts-main.ts: 不再传入 hostExecutable
- server.ts: 移除 hostExecutable 引用
...
状态：`done`；预计 2.5 人日；依赖 W03。

**实现提交：** W04 凭据 Worker 完成。
- credential-worker.ts: 隔离子进程，IPC JSONL 协议，action 调度
- credential-windows.ts: koffi DPAPI/Credential Manager 封装
- credential-store.ts: envelope v2 格式管理，原子写入
- auth-metadata.ts: 安全元数据提取（不含原始凭据）
- auth-host.ts: 改为 spawn Node credential-worker.js
...
状态：`done`；预计 2 人日；依赖 W04。

**实现提交：** W05 配置迁移完成。
- config-migration.ts: 配置 schema v1→v2 迁移
  - dryRunMigration: 分析迁移影响
  - applyMigration: 执行迁移并创建备份
  - loadConfigCompat: 兼容 v1/v2 配置加载
- upgrade.ts: 移除 auth_host_executable 迁移逻辑
- installer/main.ts: 更新必需文件列表
...
状态：`done`；预计 1 人日；依赖 W05 及旧格式双向验证已完成。

**实现提交：** W06 删除旧实现完成。
- 删除 host/devflow-host/: Go 普通进程 Host
- 删除 host/devflow-auth-host/: Go 凭据管理 Host
- 删除 host/DevFlow.WinHost/: C# Windows Host
- 删除 host/NuGet.Config: NuGet 配置
...
状态：`done`；预计 2 人日；依赖 W06。

**实现提交：** W07 回归测试完成。
- 修复 server.ts hostExecutable 引用
- 修复 credential-windows.ts 类型断言
- 修复 credential-worker.ts undefined 问题
- 修复 installer/main.ts hostName 引用
- 修复 test 文件 hostExecutable 参数
- 源码类型检查通过，集成测试通过 (8 pass, 1 skipped)
...
状态：`ready`；预计 0.5 人日，另加真实操作窗口；依赖 W07。

**W00-W07 实现已完成，可执行以下操作：**

1. 运行配置迁移：
   ```powershell
   pnpm exec tsx scripts/migrate-node-runtime.ts --config <配置路径> --dry-run
   pnpm exec tsx scripts/migrate-node-runtime.ts --config <配置路径> --apply --expected-sha256 <哈希>
   ```
...
4. 验证：
   - 检查 credential-worker.js 是否正常启动
   - 检查控制器锁是否正常获取
   - 检查进程管理是否正常工作
