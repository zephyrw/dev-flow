...
| QUALITY_REVIEW | before_human / after_human | 规划模型只读审查同一交付修订 |
...
before_human 和 after_human 各有独立 `executor_rejections`，初值为 0。每个不同交付修订的有效 changes_required 结果加 1；相同输入重复审查、网络重试、缺报告、额度或环境错误不增加次数。

本版把“连续三次执行模型代码质量有问题”固定为：**首次完整交付的审查不通过计第 1 次；执行模型整改后仍不通过计第 2 次；再次整改仍不通过计第 3 次，立即接管。不会再派发第 4 次执行模型质量交付。**

| 当前累计不合格次数 | 唯一下一步 |
| --- | --- |
| 1 | 规划模型生成完整整改计划，自动交给执行模型 |
| 2 | 更新完整整改计划，自动交给执行模型 |
| 3 | 规划模型接管修改，执行模型退出该阶段的质量整改责任 |

正常范围内的质量整改沿用原批准权限，不要求人工逐轮批准。确需改变业务需求、架构、数据结构或外部接口时，进入方案修订并由用户确认，不能用“质量优化”扩大范围。

阶段通过后保留计数历史，下一道质量关卡从 0 开始。最终整合导致第二阶段重新开启时沿用原阶段计数，不用新 epoch 把失败次数清零。功能问题修复次数不计入质量审查三次限制。
