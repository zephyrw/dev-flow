...
日期：2026-09-11。本文替代原设计中的 Windows 账户隔离、人工接入和手动串联工具要求。用户已明确暂停测试，本轮完成源码调整与安装编译，不宣称新版验收通过。
...
| `packages/service/src/launcher.ts` | 按安装目录启动、识别已有实例、并发启动锁、首次配对链接 |
...
去掉手工登记项目、填写工作流参数和导入计划 JSON 的日常按钮；“如何开始新任务”只显示一句话示例。审批链接直接选中对应任务。首次配对码由本地启动器通过 URL fragment 传入，页面读取后立即清除；不写入 HTTP 查询日志。
...
用户要求“先不要测试”。本轮只进行源码审查、必要的安装编译和配置更新，不运行单元/集成/E2E/OpenTabs、不调用 agy/GPT-6 做运行探针。之前中断的真实 agy 试验不能计为成功，历史报告均加注适用范围。

恢复测试后需验证：一句话首次接入和已有项目续接、不同会话与 worktree、不重复启动与崩溃恢复、模型指定、计划经 MCP 传递、执行过程日志、准备阶段停止、人工验收及复核提交闭环。当前验证清单明确 `current_changes_verified=false`。
