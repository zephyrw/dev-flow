...
function recoveryBlocked(
  ...
  const approved = s.engine.get(s.w.id);
  s.engine.transition(s.w.id, [approved.state], "BLOCKED", "blocked", {
    run_id: runId,
    blocker: { code: "NATIVE_RUN_FAILED", message: "运行中断" },
  });
  s.store.put("run", runId, s.w.id, {
    id: runId,
    workflow_id: s.w.id,
...
it("审查运行故障恢复仍入队审查，不改派执行", async () => {
  const s = await fixture();
  try {
    recoveryBlocked(s, "rev-fail", {
      stage: "quality_before_human",
      purpose: "quality_review",
      continuation: {
        kind: "intent_clarification",
        source_run_id: "rev-fail",
        purpose: "review",
        role: "planner",
        phase: "before_human",
        conversation_id: "review-session",
        original_text: "请补充审查结论",
      },
    });
    const next = resumeApproved(s.engine, s.w.id);
    expect(next.state).toBe("REVIEW_QUEUED");
    expect(next.stage).not.toBe("execute");
    expect(readRunContinuation(s.store, s.w.id)).toMatchObject({
      purpose: "review",
      original_text: "请补充审查结论",
      conversation_id: "review-session",
    });
...
it("范围内反馈在无 waiting 时审查失败仍回审查", async () => {
  const s = await fixture();
  try {
    recoveryBlocked(s, "rev-feedback", {
      stage: "quality_before_human",
      purpose: "quality_review",
      continuation: {
        kind: "intent_clarification",
        source_run_id: "rev-feedback",
        purpose: "review",
        role: "planner",
        phase: "before_human",
        original_text: "请补充审查结论",
    ...
    const next = s.engine.feedback(s.w.id, "按原结论继续审查", "within_plan");
    expect(next.state).toBe("REVIEW_QUEUED");
    expect(next.stage).not.toBe("execute");
...
it("规划 assigned 后运行故障恢复仍入队规划并保留原文", async () => {
  const s = await fixture();
  try {
    recoveryBlocked(s, "plan-fail", {
      stage: "planning",
      purpose: "planning",
    });
    savePlanningHandoff(s.store, s.w.id, {
      handoff_id: "handoff-assigned",
      source_run_id: "exec-ask",
      source_role: "executor",
      target_role: "planner",
      target_run_id: "plan-fail",
      status: "assigned",
      original_text: "模块边界和计划不一致",
      questions: ["B 模块是否独立？"],
    });
    ...
    const handoff = readPlanningHandoff(s.store, s.w.id);
    expect(handoff?.status).toBe("assigned");
    expect(handoff?.original_text).toBe("模块边界和计划不一致");
    const jobs = s.store
      .jobs()
      .filter((job) => job.kind === "dispatch_run")
      .map((job) => JSON.parse(job.data));
    expect(jobs.some((job) => job.purpose === "planning")).toBe(true);
  } finally {
    await cleanup(s);
  }
});

it("新独立轮次不拾取历史失败补问，也不清除用途不匹配的 staged 续接", async () => {
  const s = await fixture();
  try {
    const approved = s.engine.get(s.w.id);
    s.store.put("run", "old-failed", s.w.id, {
      id: "old-failed",
      workflow_id: s.w.id,
      plan_revision: approved.plan_revision,
      adapter: "codex",
      ...
      continuation: {
        kind: "intent_clarification",
        source_run_id: "old-failed",
        purpose: "execute",
        role: "executor",
        original_text: "旧补问不应被新轮次拾取",
      },
    });
    saveRunContinuation(s.store, s.w.id, s.w.id, {
      kind: "intent_clarification",
      source_run_id: "rev-old",
      purpose: "review",
      role: "planner",
    ...
    const consumed = (s.engine as any).consumeContinuation(
      s.w.id,
      "execute",
      "executor",
    );
