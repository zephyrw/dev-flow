...
it("完成入队包含 report_paths，详情不从清单合成 pending，drain 后状态推进", async () => {
  const { Store } = await import("../../packages/store/src/store.js");
  const {
    uniqueAttachments,
  ...
  } = await import("../../packages/evidence/src/archive-consumer.js");
  const isolated = openIsolatedDir("enqueue-reports");
  const repoInfo = await repository(isolated);
  mkdirSync(join(repoInfo.repo, ".reports"), { recursive: true });
  writeFileSync(join(repoInfo.repo, ".reports", "unit.json"), '{"ok":true}');
  const store = new Store(join(isolated, "state", "devflow.sqlite"));
  const workflowId = "wf-report-paths";
  const workspace = {
    id: "ws-1",
  ...
  store.put("workspace", workspace.id, workflowId, workspace);
  const manifest: DeliveryManifest = {
    implementations: [],
    test_executions: [
      {
  ...
  expect(uniqueAttachments(manifest, [workspace as any])).toEqual([
    { repo_id: "main", path: ".reports/unit.json" },
    { repo_id: "main", path: ".reports/missing.json" },
  ]);
  store.put("delivery", "del-unsynthesized", workflowId, {
    id: "del-unsynthesized",
    workflow_id: workflowId,
    run_id: "run-1",
    plan_revision: 1,
    plan_hash: "h",
    status: "pending",
    input_manifest_id: "man-1",
    manifest,
    submitted_at: new Date().toISOString(),
  });
  expect(
    listAttachmentRecords(store, workflowId, "del-unsynthesized"),
  ).toEqual([]);
  const job = createArchiveJobFromManifest(store, {
    deliveryId: "del-unsynthesized",
    workflowId,
    runId: "run-1",
  ...
  const pending = listAttachmentRecords(
    store,
    workflowId,
    "del-unsynthesized",
  );
  expect(pending.map((item) => item.path).sort()).toEqual([
  ...
  const afterDrain = listAttachmentRecords(
    store,
    workflowId,
    "del-unsynthesized",
  );
  expect(
    afterDrain.find((item) => item.path === ".reports/unit.json")?.state,
  ).toBe("archived");
  expect(
    afterDrain.find((item) => item.path === ".reports/missing.json")?.state,
  ).toBe("missing");
  store.close();
});

it("同一 delivery 下多仓同路径各自保留归档状态", async () => {
  const { Store } = await import("../../packages/store/src/store.js");
  const {
    uniqueAttachments,
  ...
  const isolated = openIsolatedDir("multi-repo-path");
  const store = new Store(join(isolated, "state", "devflow.sqlite"));
  const workflowId = "wf-multi-repo";
  const manifest: DeliveryManifest = {
    implementations: [],
    test_executions: [
      {
    ...
    artifacts: [
      { repo_id: "repo-a", path: ".reports/unit.json" },
      { repo_id: "repo-b", path: ".reports/unit.json" },
    ],
    acceptance_mappings: [],
    unfinished_items: [],
    plan_conflicts: [],
  ...
  store.put("delivery", "del-multi", workflowId, {
    id: "del-multi",
    workflow_id: workflowId,
    run_id: "run-1",
    plan_revision: 1,
  ...
  store.put(
    "attachment_archive",
    attachmentArchiveKey("del-multi", "repo-a", ".reports/unit.json"),
    workflowId,
    {
      delivery_id: "del-multi",
      repo_id: "repo-a",
  ...
  store.put(
    "attachment_archive",
    attachmentArchiveKey("del-multi", "repo-b", ".reports/unit.json"),
    workflowId,
    {
      delivery_id: "del-multi",
      repo_id: "repo-b",
  ...
  const records = listAttachmentRecords(store, workflowId, "del-multi");
  expect(records).toHaveLength(2);
  const repoA = records.find((item) => item.repo_id === "repo-a");
  const repoB = records.find((item) => item.repo_id === "repo-b");
  expect(repoA?.path).toBe(".reports/unit.json");
  expect(repoA?.state).toBe("archived");
  expect(repoB?.path).toBe(".reports/unit.json");
  expect(repoB?.state).toBe("missing");
  store.close();
});

function openIsolatedDir(caseName: string) {
  const base = join(process.env.TEMP || tmpdir(), "dfr4", "c", caseName);
