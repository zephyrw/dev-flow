...
test("E2E-U05 保存后续配置显示下次派发生效文案，且不默认勾选暂停", async ({
  ...
  await expect(drawer.getByText("完成可审计安全暂停后再应用新规格")).toHaveCount(0);
  await expect(
    drawer.getByRole("button", { name: "保存，下次派发生效" }),
  ).toBeVisible();
  ...
  if (await success.count()) {
    await expect(success).toContainText(/已保存；/);
...
test("E2E-U06 任务抽屉不把继续开发当成统一文案", async ({ page }) => {
  ...
  const drawer = page.getByRole("dialog", { name: "工具与模型配置" });
  await expect(drawer).toBeVisible();
  await expect(drawer.getByRole("button", { name: "继续开发", exact: true })).toHaveCount(0);
});
...
async function pauseSwitchReviewer(
  ...
  const reviewRow = drawer.locator(".ms-role-row").filter({ hasText: "代码审查" });
...
test("E2E-U11 两窗口并发改任务配置冲突提示", async ({ page, context }) => {
  ...
  await waitAccessStatus(
    drawer.locator(".ms-editor").nth(1),
    "已验证可访问",
  );
...
test("E2E-U07 两个审查阶段分别暂停换模型均回原审查阶段", async ({ page }) => {
  ...
  await expect
    .poll(
      async () => (await workflowDetail(page, id)).workflow,
      { timeout: 30000 },
    )
...
test("E2E-R21 首次指定质量修复者、修改已有覆盖、活跃修复中保存未来选择", async ({
  ...
  const openCard = drawer.locator(".ms-role-row").filter({ hasText: "尚未指定覆盖" });
  ...
  await expect(drawer.locator(".ms-success")).toContainText("只影响后续修复轮次", {
    timeout: 20000,
  });
  const firstQuality = drawer.locator(".ms-role-row").filter({ hasText: "before_human" });
  ...
  await expect(drawer.getByText("尚未指定覆盖")).toBeVisible();
  const covered = drawer.locator(".ms-role-row").filter({ hasText: "after_human" });
  ...
  const functional = drawer.locator(".ms-role-row").filter({ hasText: "功能修复" });
