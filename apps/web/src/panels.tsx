...
function AttachmentArchiveStatus({ items }: { items?: any[] }) {
  const records = Array.isArray(items) ? items.filter((item) => item && typeof item.path === "string" && item.path.trim()) : [];
        ...
        >
          <div className="test-case-row">
            <span className={"badge " + item.state}>
              {typeof item.state === "string" ? attachmentLabels[item.state] ?? item.state : "状态未知"}
            </span>
