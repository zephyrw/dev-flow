...
import { closeQualityRepairBatch, ensureQualityRepairBatch } from "./repair-model-service.js";
...
export class QualityCoordinator {
  ...
  private persistFull(transfer: QualityTransfer) {
    ...
    if (transfer.decision.action === "pass") {
      ...
      closeQualityRepairBatch(this.store, transfer.workflow_id, phase);
    } else {
      const failures = [...(gate.failed_repair_review_ids ?? [])];
      if (
      ...
      gate.status = "rejected";
      ensureQualityRepairBatch(this.store, transfer.workflow_id, phase, transfer.review_run_id);
      gate.cycle++;
  ...
  private persistBackfill(transfer: QualityTransfer) {
    if (transfer.decision.action === "pass") closeQualityRepairBatch(this.store, transfer.workflow_id, transfer.phase);
    else if (transfer.assignment) ensureQualityRepairBatch(this.store, transfer.workflow_id, transfer.phase, transfer.review_run_id);
