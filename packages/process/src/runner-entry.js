...
function parseIncoming(raw) {
    ...
    if (raw.type === "stop") {
        ...
        const stopSeconds = typeof raw.stop_seconds === "number" &&
 ...
 */
function stopTool(graceMs, fromDisconnect) {
    if (!tool || stopping)
        return;
    stopping = true;
    const pid = tool.pid;
...
function handleStop(msg) {
    const graceMs = msg.stop_seconds !== undefined
