...
import { z } from "zod";
import { planningBridgeInstructions } from "../../core/src/conversation-guidance.js";
...
const plannerGuidance = planningBridgeInstructions();
...
for (const tool of (await discovery.listTools()).tools) {
  ...
  const description = [tool.description, plannerGuidance].filter(Boolean).join(" ");
  server.registerTool(tool.name, { description, inputSchema: schema, annotations: tool.annotations },
    async args => {
