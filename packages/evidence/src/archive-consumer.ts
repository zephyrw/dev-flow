...
function rememberRecord(
  ...
  const key = JSON.stringify([record.delivery_id, record.path]);
...
function hydrateArchiveRecord(
  ...
  return {
    delivery_id: value.delivery_id || parsed?.deliveryId || "",
    repo_id: value.repo_id || parsed?.repoId || "main",
    path: value.path || parsed?.path || "",
...
export function listAttachmentRecords(
  ...
  for (const delivery of store.list<Delivery>("delivery", workflowId)) {
    ...
    if (!delivery.manifest) continue;
    for (const item of uniqueAttachments(delivery.manifest, [])) {
...
function uniqueAttachments(
  ...
  for (const artifact of manifest.artifacts ?? []) {
    ...
    const rec = artifact as { path?: string; file?: string; repo_id?: string };
  ...
  function add(repoId: string, path: string) {
    const key = repoId + "\0" + path;
    if (seen.has(key)) return;
    seen.add(key);
    items.push({ repo_id: repoId, path });
  }
}
...
function collectOutboxJobs(store: Store): ArchiveOutboxItem[] {
  ...
  for (const job of store.list<ArchiveOutboxItem>(OUTBOX_KIND)) {
    if (job?.delivery_id && job.items?.length)
      jobs.set(job.delivery_id, job);
  }
  ...
  for (const delivery of store.list<Delivery>("delivery")) {
    ...
    const pending = (delivery.attachment_status ?? []).filter(
      (record) => record.path && record.state !== "archived",
    );
...
function parseQueuedOutbox(
  ...
  return {
    ...
    workflow_id:
      typeof rec.workflow_id === "string" ? rec.workflow_id : workflowId,
...
async function archiveOne(
  ...
  store.put(RECORD_KIND, entry.key, entry.job.workflow_id, {
    ...result,
    path: result.path || entry.item.path,
  });
}
...
function resolveWorkspaceRoot(
  ...
  const list =
    workspaces?.length
      ? workspaces
      : store.list<Workspace>("workspace", workflowId);
  return list.find((workspace) => workspace.repo_id === repoId)?.root ??
    (list.length === 1 ? list[0]!.root : undefined);
}
