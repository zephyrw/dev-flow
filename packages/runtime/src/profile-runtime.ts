...
export class ProfileRuntime {
  ...
  private readContinuation(run: Run): RunContinuation | undefined {
    ...
    const purpose =
      run.purpose === "quality_review"
        ...
        : run.purpose === "planning"
          ? "planning"
          : ["implement", "functional_fix", "planner_takeover"].includes(
                run.purpose ?? "",
              )
            ? "execute"
            : undefined;
    for (const value of [
      ...
      const source = this.engine.store.get<Run>(
        "run",
        continuation.source_run_id,
      );
  ...
  private conversationToResume(run: Run, purpose: string, sessionKey: string) {
