...
export class ProfileRuntime {
  ...
  private executeMaterials(w: Workflow, run: Run) {
    const plan = this.engine.plan(w.id);
    return applyContinuationMaterials(
      {
        instructions:
  ...
  private readContinuation(run: Run): RunContinuation | undefined {
    // Workflow-scoped continuations are scheduler input, never runtime fallback.
    // The scheduler binds a matching continuation to the newly dispatched Run.
    const purpose = run.purpose === "quality_review"
      ? "review"
      : run.purpose === "planning"
        ? "planning"
        : ["implement", "functional_fix", "planner_takeover"].includes(run.purpose ?? "")
          ? "execute"
          : undefined;
    for (const value of [
      run.continuation,
      this.engine.store.get("run_continuation", run.id),
    ]) {
      const continuation = asRunContinuation(value);
      if (!continuation || continuation.purpose !== purpose) continue;
      const source = this.engine.store.get<Run>("run", continuation.source_run_id);
  ...
  private conversationToResume(
    run: Run,
    purpose: string,
    sessionKey: string,
  ) {
    const stored = this.engine.store.get<{ id: string }>(
      "native_conversation",
      sessionKey,
    );
    const continuation = this.readContinuation(run);
    return selectConversationToResume({
      purpose,
      continuation: {
        conversation_id:
          continuation?.conversation_id ??
          (continuation ? undefined : run.continuation_conversation_id),
      },
      planningSession: purpose === "planning" ? stored : undefined,
  ...
  private async invoke(
    ...
    const sessionKey =
      ...
      (["implement", "plan_self_check", "functional_fix"].includes(purpose)
        ...
        : purpose);
    const continuation = this.readContinuation(run);
    const previous = this.conversationToResume(run, purpose, sessionKey);
    if (continuationSessionRequired(purpose, continuation) && !previous)
      throw normalizeRuntimeFailure(
        new FlowError("NATIVE_RUN_FAILED", "原角色会话不可恢复", 422),
      );
    const context = {
      workflowId: w.id,
      runId: run.id,
