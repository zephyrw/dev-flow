...
async function runRealBenchmark() {
  ...
  try {
    ...
    // 4. 真实 HTTP 请求测 API
    console.log(
      "开始进行真实 metadata 与 health API 延迟测量 (100 次真实请求)...",
    );
    ...
    console.log(
      "真实 /api/workflows 延迟 P95:",
      workflowsLatency.p95Ms,
      "ms (标准 <=200ms)",
    );
    ...
    console.log(
      "真实哈希吞吐:",
      hashThroughput.throughputGBPerSec,
      "GB/s (标准 >=1.0 GB/s)",
    );
    ...
    const totalCpuDelta = cpuDeltaService;

    // 单逻辑核心占用率 (%)
    const cpuPercent = Number(((totalCpuDelta / wallSeconds) * 100).toFixed(2));
    const totalRssMiB = m1.rssMiB;

    console.log(`空闲/测试期间进程 CPU 增量占比: ${cpuPercent}% (标准 <=1.0%)`);
    console.log(
      `被测平台总 WorkingSet/RSS: ${totalRssMiB} MiB (服务 ${m1.rssMiB}M, 标准 <=200MiB)`,
    );
    ...
    const thresholds = {
      ...
      platform_memory_rss_mib: {
        actual: totalRssMiB,
        limit: 200,
        pass: totalRssMiB <= 200,
      },
      metadata_api_p95_ms: {
        actual: workflowsLatency.p95Ms,
        limit: 200,
        pass: workflowsLatency.p95Ms <= 200,
      },
      health_api_p95_ms: {
        actual: healthLatency.p95Ms,
        limit: 200,
        pass: healthLatency.p95Ms <= 200,
      },
      hash_throughput_gb_per_sec: {
        actual: hashThroughput.throughputGBPerSec,
        limit: 1.0,
        pass: hashThroughput.throughputGBPerSec >= 1.0,
      },
    ...
    const report = {
      ...
      macos_arm64: {
        status: "blocked",
        reason:
          "本机为 Windows x64 平台，无真实 macOS arm64 环境，按计划 441 节规范严禁伪造数据",
      },
    };
    ...
    writeFileSync(
      join(outDir, "bench-report.json"),
      JSON.stringify(report, null, 2),
      "utf8",
    );
    writeFileSync(
      resolve(".cache/bench-report.json"),
      JSON.stringify(report, null, 2),
      "utf8",
    );
    console.log(
      "=== 基准测试完成，报告已归档至 docs/test/evidence/devflow-final-20260916/bench-report.json ===",
    );
    console.log("总体裁定 (Verdict):", report.verdict);
  } catch (benchErr) {
    const serviceLog = existsSync(serviceLogPath)
      ? readFileSync(serviceLogPath, "utf8")
      : "";
    ...
    if (serviceProc && serviceProc.pid) {
      try {
        process.kill(serviceProc.pid);
      } catch {}
      try {
        execSync(`taskkill /PID ${serviceProc.pid} /T /F`, { stdio: "ignore" });
      } catch {}
    }
    try {
